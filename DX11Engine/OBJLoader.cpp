...
MeshData OBJLoader::Load(const char* filename, bool invertTexCoords)
{
	ID3D11Device* _pd3dDevice = Graphics::GetDevice();
	...
	if(!binaryInFile.good())
	{
		...
		else
		{
			...
			meshData.iBuffer = Buffer::CreateBuffer<WORD>(indicesArray, numMeshIndices, BufferBindFlag::Index);

#pragma once

#ifndef SIMPLEX_NOISE
	#define SIMPLEX_NOISE

#include <stdlib.h>
#include <string>
#include "Math.h"
#include "Log.h"

class SimplexNoise // based on https://solarianprogrammer.com/2012/07/18/perlin-noise-cpp-11/
{
#define FLOOR(x) ( ((x)>0) ? ((int)x) : (((int)x)-1) )

    unsigned char perm[512] = // default perm values
    {
    	151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,
    	8,99,37,240,21,10,23,190,6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,
    	35,11,32,57,177,33,88,237,149,56,87,174,20,125,136,171,168,68,175,74,165,71,
    	134,139,48,27,166,77,146,158,231,83,111,229,122,60,211,133,230,220,105,92,41,
    	55,46,245,40,244,102,143,54,65,25,63,161,1,216,80,73,209,76,132,187,208,89,18,
    	169,200,196,135,130,116,188,159,86,164,100,109,198,173,186,3,64,52,217,226,
    	250,124,123,5,202,38,147,118,126,255,82,85,212,207,206,59,227,47,16,58,17,
    	182,189,28,42,223,183,170,213,119,248,152,2,44,154,163, 70,221,153,101,155,
    	167, 43,172,9,129,22,39,253,19,98,108,110,79,113,224,232,178,185, 112,104,
    	218,246,97,228,251,34,242,193,238,210,144,12,191,179,162,241,1,51,145,235,
    	249,14,239,107,49,192,214,31,181,199,106,157,184, 84,204,176,115,121,50,45,
    	127,4,150,254,138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,215,
    	61,156,180,151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,
    	30,69,142,8,99,37,240,21,10,23,190,6,148,247,120,234,75,0,26,197,62,94,252,
    	219,203,117,35,11,32,57,177,33,88,237,149,56,87,174,20,125,136,171,168,68,175,
    	74,165,71,134,139,48,27,166,77,146,158,231,83,111,229,122,60,211,133,230,220,
    	105,92,41,55,46,245,40,244,102,143,54,65,25,63,161,1,216,80,73,209,76,132,
    	187,208,89,18,169,200,196,135,130,116,188,159,86,164,100,109,198,173,186,3,
    	64,52,217,226,250,124,123,5,202,38,147,118,126,255,82,85,212,207,206,59,227,
    	47,16,58,17,182,189,28,42,223,183,170,213,119,248,152,2,44,154,163,70,221,
    	153,101,155,167,43,172,9,129,22,39,253,19,98,108,110,79,113,224,232,178,185,
    	112,104,218,246,97,228,251,34,242,193,238,210,144,12,191,179,162,241,81,51,
    	145,235,249,14,239,107,49,192,214,31,181,199,106,157,184,84,204,176,115,121,
    	50,45,127,4,150,254,138,236,205,93,222,114,67,29,24,72,243,141,128,195,78,66,
    	215,61,156,180
    };

public:
	SimplexNoise() = default;
	SimplexNoise(int seed)
	{
        SetSeed(seed);
	}

	void SetSeed(int seed)
	{
		srand(seed);

		for (int i = 0; i < 256; ++i) // fill perm with values based on set seed.
		{
            int permNum = rand() % 256;
            perm[i] = (unsigned char)permNum;
            perm[i + 256] = (unsigned char)permNum;
		}

        std::string str = "perm = ";

        for (int i = 0; i < 256; i++)
        {
            str += std::to_string((int)(perm[i])) + " ";
        }

        Logger::ENGINE_LOG(Logger::Info, str);
	}

    float Grad(int hash, float x, float y, float z)
	{
        int h = hash & 15;

        float u = h < 8 ? x : y;
        float v = h < 4 ? y : h == 12 || h == 14 ? x : z;
        return ((h & 1) == 0 ? u : -u) + ((h & 2) == 0 ? v : -v);
	}

    float GetNoise3D(float x, float y, float z)
	{
        int _x = (int)FLOOR(x) & 255;
        int _y = (int)FLOOR(y) & 255;
        int _z = (int)FLOOR(z) & 255;

        x -= FLOOR(x);
        y -= FLOOR(y);
        z -= FLOOR(z);

        float u = Math::Fade(x);
        float v = Math::Fade(y);
        float w = Math::Fade(z);

        int a = perm[_x] + _y;
        int aa = perm[a] + _z;
        int ab = perm[a + 1] + _z;
        int b = perm[_x + 1] + _y;
        int ba = perm[b] + _z;
        int bb = perm[b + 1] + _z;

        float val = Math::Lerp(w, Math::Lerp(v, Math::Lerp(u, Grad(perm[aa], x, y, z), Grad(perm[ba], x - 1, y, z)),
            Math::Lerp(u, Grad(perm[ab], x, y - 1, z), Grad(perm[bb], x - 1, y - 1, z))),
            Math::Lerp(v, Math::Lerp(u, Grad(perm[aa + 1], x, y, z - 1), Grad(perm[ba + 1], x - 1, y, z - 1)),
            Math::Lerp(u, Grad(perm[ab + 1], x, y - 1, z - 1), Grad(perm[bb + 1], x - 1, y - 1, z - 1))));

        return (val + 1.f) / 2.f;
	}

    float GetNoise3D(float x, float y, float z, int octaves, float lacunarity, float persistance, float mapScale)
	{
        float freq = 1.f, apl = 1.f, sum = 0.f;

        for (int i = 0; i < octaves; i++)
        {
            sum += GetNoise3D(x * mapScale * freq, y * mapScale * freq, z * mapScale * freq) * apl;
            freq *= lacunarity;
            apl *= persistance;
        }

        return sum; // normalize val
	}
};

#endif